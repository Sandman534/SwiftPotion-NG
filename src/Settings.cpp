...
constexpr auto blacklist_path = L"Data/SKSE/Plugins/SwiftPotionNG_Blacklist.ini";
...
void Settings::LoadSettings() {
	...
	// Load Blacklist
	LoadBlacklistINI(&blacklistForms);
...
void Settings::LoadBlacklistINI(std::unordered_set<int> *set) {
	CSimpleIniA iniBlacklist;
	iniBlacklist.SetUnicode();
	iniBlacklist.LoadFile(blacklist_path);

	std::list<CSimpleIniA::Entry> sectionList;
	iniBlacklist.GetAllSections(sectionList);

	for (CSimpleIniA::Entry strESP : sectionList) {
		std::string esp(strESP.pItem);
		auto index = RE::TESDataHandler::GetSingleton()->GetModIndex(esp);
		
		if (index.value() != 0xFF) {
			std::list<CSimpleIniA::Entry> entryList;
			iniBlacklist.GetAllKeys(esp.c_str(), entryList);

			for (CSimpleIniA::Entry str : entryList) {
				std::string value = iniBlacklist.GetValue(strESP.pItem, str.pItem);
	...
	// Log
	logger::debug("Loaded {} Blacklisted Effects", set->size());
}

std::string Settings::HotkeyString(int iNum, std::string sName) {
...
void Settings::SetupEffects() {
	...
	for (auto potion : potionList) {
		...
		for (auto& foundEffect : foundPotion->effects) {
...
void Settings::DefaultAutoData(PotionData &SystemData, std::string sType, std::string sEffectName) {
	// General defaults
...
// Settings Details
void Settings::detail::get_value(CSimpleIniA& a_ini, bool& a_value, const char* a_section, const char* a_key) {
...
void Settings::detail::get_value(CSimpleIniA& a_ini, std::string& a_value, const char* a_section, const char* a_key) {
...
void Settings::detail::get_value(CSimpleIniA& a_ini, int& a_value, const char* a_section, const char* a_key) {
...
void Settings::detail::get_value(CSimpleIniA& a_ini, float& a_value, const char* a_section, const char* a_key) {
	std::string s = std::to_string(a_value);
	char const *pchar = s.c_str();
	if (a_ini.KeyExists(a_section, a_key))
...
void Settings::detail::set_value(CSimpleIniA& a_ini, bool a_value, const char* a_section, const char* a_key) {
	a_ini.SetBoolValue(a_section, a_key, a_value);
};

void Settings::detail::set_value(CSimpleIniA& a_ini, std::string a_value, const char* a_section, const char* a_key) {
	char const *pchar = a_value.c_str();
	a_ini.SetValue(a_section, a_key, pchar);
};

void Settings::detail::set_value(CSimpleIniA& a_ini, int a_value, const char* a_section, const char* a_key) {
	std::string s = std::to_string(a_value);
	char const *pchar = s.c_str();
	a_ini.SetValue(a_section, a_key, pchar);
};

void Settings::detail::set_value(CSimpleIniA& a_ini, float a_value, const char* a_section, const char* a_key) {
	std::string s = std::to_string(a_value);
	char const *pchar = s.c_str();
	a_ini.SetValue(a_section, a_key, pchar);
};
...
// Potion Data
bool PotionData::solve(std::string s) {
	return s != "false";
}

std::string PotionData::Serialize() {
...
void PotionData::Deserialize(std::string sData) {
	size_t pos = 0;
	std::string token;
